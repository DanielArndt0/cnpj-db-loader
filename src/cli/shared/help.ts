...
${theme.section("Recommended flow")}
  ...
  ${theme.command("cnpj-db-loader import ./downloads/extracted")}

${theme.section("Notes")}
  ${theme.muted("Use inspect first to understand whether the input is still zipped or already extracted.")}
  ${theme.muted("Generate the schema only when you need to create the database structure. Import is the recurring step after validation.")}
