...
export async function sanitizeInputDirectory(
  ...
  if (!validation.ok) {
    throw new ValidationError(
      `O diretório de entrada não está pronto para a sanitização. ${validation.errors.join(" ")}`,
    );
  }
  ...
  if (plan.totalFiles === 0) {
    throw new ValidationError(
      "Nenhum arquivo de dataset validado reconhecido foi encontrado para a sanitização.",
    );
  }
  ...
  return {
    ...
    files: fileSummaries,
    warnings: [
      "A sanitização normaliza os arquivos de origem da Receita para UTF-8, remove bytes NUL em nível de byte e remove caracteres de controle problemáticos antes de iniciar a carga no PostgreSQL.",
